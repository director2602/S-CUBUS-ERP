...
export async function createStudent(formData: FormData) {
  ...
  if (scid) {
    const existing = db
      ...
      .get();
    if (existing) throw new Error(`SCID "${scid}" is already assigned to another student.`);
  }
  if (sathiiKey) {
    const existing = db
      .select()
      ...
      .get();
    if (existing) throw new Error(`SATHII KEY "${sathiiKey}" is already assigned to another student.`);
  }
